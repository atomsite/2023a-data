...
def radioactivity(image):
  """
  
  """
  air_wet_thresh = 200.0
  wet_fe_thresh  = 2000.0
  rho = image.rho.data
  rad = image.heat.data
  ni = image.xnumx
  nj = image.ynumy
  ...
  for i in range(ni):
    for j in range(nj):
      if rho[i,j] > air_wet_thresh:
        sum_rho   += rho[i,j]
        ncells_pl += 1
      if rho[i,j] > air_wet_thresh and rho[i,j] < wet_fe_thresh: 
        heat_mass_cell = rad[i,j]
        cell_mass = rho[i,j] * vol
        heat_tot_al26 += heat_mass_cell * cell_mass
      elif rho[i,j] >= wet_fe_thresh:
        heat_mass_cell = rad[i,j]
        cell_mass = rho[i,j] * vol
        heat_tot_fe60 += heat_mass_cell * cell_mass
  # Calculate mass and volume of planetesimal
  avg_rho = sum_rho / ncells_pl
  pl_vol = vol * ncells_pl
  pl_mass = avg_rho * pl_vol
  # Calculate radiation by unit mass and unit vol, store in dict
  heating = {}
  heating["al26_tot"]  = heat_tot_al26
  heating["al26_mass"] = heat_tot_al26 / pl_mass
  heating["al26_vol"]  = heat_tot_al26 / pl_vol
  heating["fe60_tot"]  = heat_tot_fe60
  heating["fe60_mass"] = heat_tot_fe60 / pl_mass
  heating["fe60_vol"]  = heat_tot_fe60 / pl_vol
  # Return results dict
  return heating
